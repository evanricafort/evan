...
if ((BASH_VERSINFO[0] < 5 || (BASH_VERSINFO[0] == 5 && BASH_VERSINFO[1] < 2))); then
    echo -e "${RED}Error: This script requires Bash version 5.2 or higher.${NC}"
    exit 1
fi
...
# Option parsing
# Fix getopt parsing for Bash 5.2 compatibility
TEMP=$(getopt -o t:u:1234ankhb --long pret -n "$0" -- "$@")
if [ $? != 0 ]; then
